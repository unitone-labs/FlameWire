...
class MinerScorer:
    ...
    def _metrics(self, last_n_checks: List[bool], last_n_response_times: List[float]) -> Tuple[float, float, int]:
        ...
        for check in reversed(checks):
            if not check:
                ...
                break
        return success_rate, avg_time, fail_streak

    def score(self, last_n_checks: List[bool], last_n_response_times: List[float]) -> float:
        success_rate, avg_time, fail_streak = self._metrics(last_n_checks, last_n_response_times)
    ...
    def score_with_metrics(
        ...
        """Return score along with success rate, average time and fail streak."""
        success_rate, avg_time, fail_streak = self._metrics(last_n_checks, last_n_response_times)
        score = self.score(last_n_checks, last_n_response_times)
        return score, success_rate, avg_time, fail_streak
