...
class MinerScorer:
    def __init__(self, window_size: int = 25, penalty_per_fail: float = 0.05, max_penalty: float = 0.2, insufficient_data_penalty: float = 0.2):
        self.window_size = window_size
        self.penalty_per_fail = penalty_per_fail
        self.max_penalty = max_penalty
        self.insufficient_data_penalty = insufficient_data_penalty
    ...
    def score(self, last_n_checks: List[bool], last_n_response_times: List[float]) -> float:
        ...
        has_insufficient_data = len(last_n_checks) < self.window_size
        ...
        if has_insufficient_data:
            success_rate *= (1 - self.insufficient_data_penalty)
    ...
    @staticmethod
    def quick_score(last_n_checks: List[bool], last_n_response_times: List[float], window_size: int = 25, penalty_per_fail: float = 0.05, max_penalty: float = 0.2, insufficient_data_penalty: float = 0.2) -> float:
        scorer = MinerScorer(window_size, penalty_per_fail, max_penalty, insufficient_data_penalty)
